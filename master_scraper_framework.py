...
def collection(url, state, type, year):
    ...
    def clean_data(dataframe, type, year):         
    ...
    if type in ["senate", "governor"]:
        ...
        #df = clean_data(df, type, year)
        #return df
    elif type in ["general"] and year not in ["2024"]:
        print("headers")
        print()
...
#for year, type, state, url in master_url.items():
    #df = collection(url, state, type, year)
